...
import {availableTools, getEnabledTools,enableTools,getEnabledToolNames,resetTools} from "../../config/tool.config.ts";
    ...
    const result = await aiService.sendMessage(
      ...
      (chunk) => {
        if (isFirstChunk) {
          spinner.stop();
          console.log("\n");
          const header = chalk.green.bold("🤖 Assistant(with tools):");
          console.log(header);
...
async function getConversationTitle(conversationId,userInput,messageCount){
    if(messageCount === 1){
        const title = userInput.slice(0,50) + (userInput.length > 50 ? "..." : "")
        await chatService.updateTitle(conversationId,title)
    }
...
export async function startToolChat(conversationId: null){
    try {
        ...
        intro(
            boxen(
                chalk.bold.cyan("🛠️ Orryn AI - Tool Calling Mode"),{
                    padding: 1,
                    borderStyle: "double",
                    borderColor: "cyan",
                }
